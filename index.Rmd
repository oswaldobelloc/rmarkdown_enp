...
`r colorize("R", "blue")` proporciona la función `SIGN.test(x, y = NULL, md = 0, alternative = "two.sided", conf.level = 0.95, \dotsc)` disponible en el paquete `BSDA` que permite ejecutar la prueba del signo para una muestra (desarrollada acá) y dos muestras relacionadas (tratada en el siguiente núcleo temático). Los parámetros de esta función se definen en la tabla \@ref(tab:par-prueba-signo).
...
Donde $F_{\chi_{k-1}^{2}}\left(\chi^{2} \right)$ es la función de distribución acumula de una variable aleatoria ji-cuadrado con $k-1$ grados de libertad, la cual se encuentra ampliamente tabulada. La distribución base de `r colorize("R", "blue")` proporciona las siguientes funciones, las cuales permiten evaluar estas y otras funciones relacionadas con esta distribución de probabilidad:
...
qplot(lp, geom = 'blank') +   
  geom_line(aes(y = ..density..), colour=26, stat = 'density', 
            size = 1, alpha = .6) + 
  geom_histogram(aes(y = ..density..), fill = 28, col = "black",
                 alpha = 0.3,
...
hist(
  lp,
  breaks = c(-Inf, 10.32745, 10.39883, 10.4522, 10.5, 
...
curve(
  dnorm(x, mean = 10.5, sd = 0.15), add = TRUE, col = "red", lwd = 1
)
```
...
plot(ecdf(reflectancia), xlim = c(0,1), verticals = TRUE, 
...
Note en la salida anterior que el valor del estadístico $R = `r r`$. Mientras que, el $p_{valor} = `r p.valor`$, por lo tanto las puntuaciones de agresión de acuerdo al orden en que fueron colectadas tienen un comportamiento aleatorio, lo que implica que las discusiones no tienen efecto en el nivel de agresión en las sesiones de juego.
...
Como se observa en la salida anterior el  $p_{valor} = `r p.valor`$, lo que indica que disposición de hombres y mujeres en una fila enfrente de la taquilla del teatro no es aleatorio. Note que $R = `r r` > \mu_R = `r  mu_R`$, por lo que el $p_{valor}=2P\left(R \geq r \right)$.
