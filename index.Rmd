...
Wilcoxon determinó la distribución exacta del estadístico $T^+$ asumiendo que si $x_1, x_2, \dotsc,x_n$ representa una muestra de una variable aleatoria continua $X$, $x_i \neq \widetilde{\mu}_{0}$, para $i=1, 2, \dotsc, n$, es decir los valores muestrales eran diferente a la mediana propuesta en la hipótesis nula. Además, si $\left(|d_r| \right)$ y $\left(|d_s| \right)$ son los valores absolutos de dos diferencias en la muestra, esto es, $\left(|d_r| \right) \neq \left(|d_s| \right)$, para $r \neq s$.
...
$$
\begin{equation}
V \left(T^+| \widetilde{\mu}= \widetilde{\mu}_{0} \right)=\frac{n(n+1)(2n+1)}{4}.
...
$$
\begin{equation}
Z= \frac{T^+ \pm 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}}} \simeq N \left(\mu=0, \sigma=1\right).
(\#eq:z-wilcoxon1)
\end{equation}
$$
...
$$
\begin{equation}
p_{valor} = 1 - F_N \left(Z= \frac{T^+ - 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}}}; \; \mu = 0, \; \sigma = 1 \right).
(\#eq:pusz-wilcoxon1)
\end{equation}
$$
...
$$
\begin{equation}
p_{valor} = F_N \left(Z= \frac{T^+ + 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}}}; \; \mu = 0, \; \sigma = 1 \right).
(\#eq:puiz-wilcoxon1)
\end{equation}
$$
...
p_{valor}=
\begin{cases} 
2\left[1 - F_N \left(Z= \frac{T^+ - 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}}}; \; \mu = 0, \; \sigma = 1 \right)], \; \textit{si} \; t^+ > \frac{n \left(n+1 \right)}{4} \\ 
F_N \left(Z= \frac{T^+ + 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}}}; \; \mu = 0, \; \sigma = 1 \right), \; \textit{si} \; t^+ \leq \frac{n \left(n+1 \right)}{4} 
 \end{cases}.
(\#eq:pbl-wilcoxon1)
\end{equation}
$$
...
$$
\begin{equation}
V \left(T^+| \widetilde{\mu}= \widetilde{\mu}_{0} \right)=\frac{n(n+1)(2n+1)}{4}-\sum_{j=1}^{g} \frac{t_i \left(t_i^2-1 \right)}{48}.
(\#eq:varianzac-wilcoxon1)
\end{equation}
$$

Donde $g$ es el número de grupos y $t_j$ es el número de observaciones empatadas en el grupo $j$. De lo anterior, el estadístico dado en la ecuación \@ref(eq:z-wilcoxon1) se redefine de la siguiente manera,

$$
\begin{equation}
Z= \frac{T^+ \pm 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}-\sum_{j=1}^{g} \frac{t_i \left(t_i^2-1 \right)}{48}}} \simeq N \left(\mu=0, \sigma=1\right).
(\#eq:zc-wilcoxon1)
\end{equation}
$$
...
$$
\begin{equation}
p_{valor} = 1 - F_N \left(Z= \frac{T^+ - 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}-\sum_{j=1}^{g} \frac{t_i \left(t_i^2-1 \right)}{48}}}; \; \mu = 0, \; \sigma = 1 \right).
(\#eq:puszc-wilcoxon1)
\end{equation}
$$
...
$$
\begin{equation}
p_{valor} = F_N \left(Z= \frac{T^+ + 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}-\sum_{j=1}^{g} \frac{t_i \left(t_i^2-1 \right)}{48}}}; \; \mu = 0, \; \sigma = 1 \right).
(\#eq:puszc-wilcoxon1)
\end{equation}
$$
...
p_{valor}=
\begin{cases} 
2\left[1 - F_N \left(Z= \frac{T^+ - 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}-\sum_{j=1}^{g} \frac{t_i \left(t_i^2-1 \right)}{48}}}; \; \mu = 0, \; \sigma = 1 \right)], \; \textit{si} \; t^+ > \frac{n \left(n+1 \right)}{4} \\ 
F_N \left(Z= \frac{T^+ + 0,5-\frac{n(n+1)}{4}}{\sqrt{\frac{n(n+1)(2n+1)}{4}-\sum_{j=1}^{g} \frac{t_i \left(t_i^2-1 \right)}{48}}}; \; \mu = 0, \; \sigma = 1 \right), \; \textit{si} \; t^+ \leq \frac{n \left(n+1 \right)}{4} 
 \end{cases}.
(\#eq:pbl-wilcoxon1)
\end{equation}
$$

Conmover (1999, pág. 353) propone el siguiente estadístico de prueba 
...
Si $n$ es grande y hay empates en la muestra, el estadístico dado en la ecuación \@ref(eq:T-wilcoxon1) tiene $E \left(T \;| \; \widetilde{\mu}= \widetilde{\mu}_{0} \right) =0$; y varianza
...
$$
\begin{equation}
p_{valor} = 1 - F_N \left(Z= \frac{T - 0,5}{\sqrt{\frac{n(n+1)(2n+1)}{6}}}; \; \mu = 0, \; \sigma = 1 \right).
(\#eq:pusz-wilcoxon1)
\end{equation}
$$
...
$$
\begin{equation}
p_{valor} =F_N \left(Z= \frac{T + 0,5}{\sqrt{\frac{n(n+1)(2n+1)}{6}}}; \; \mu = 0, \; \sigma = 1 \right).
(\#eq:pusz-wilcoxon1)
\end{equation}
$$
...
tabla_co <- tibble(
  i = 1:length(co),
  co = co,
  diferencia = co - mediana
) %>% 
  mutate(
    rango = dplyr::if_else(
      diferencia == 0, NA_real_, 
      sign(diferencia) * 
      rank(
...
kableExtra::kbl(
  tabla_co,
  col.names = c(
    "$i$", "Concentración de octano", "$D_i$", "$R_i$"
  ),
  format.args = list(decimal.mark = ",", big.mark = "."),
  booktabs = TRUE,
  ...
  row_spec(which(tabla_co$diferencia == 0), color = "red",
           strikeout = TRUE) %>%
  kable_classic_2() %>%
  scroll_box(width = "100%", height = "400px")
```
...
En el siguiente script se muestra el calculo  del estadístico $T^+$ y el $p_{valor}$ para una prueba unilateral superior.
...
Como se muestra en la salida anterior, el estadístico $T^+=`formato(mas)`$. Mientras que el $p_{valor}=`formato(p.valor)`$, lo cual implica que hay suficiente evidencia estadística para asegurar con un 99\% de que la mediana de la concentración de octano es superior a 98.

Note que el $p_valor$ obtenido con la prueba del signo (0,0065) en el ejemplo \@ref(exm:ejemp1-signo1) no difiere mucho del obtenido con esta prueba (0,0021). En ambos casos se rechaza la hipótesis nula, dado que $p_{valor} < 0,01$.
