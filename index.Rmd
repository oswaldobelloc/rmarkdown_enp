...
```{r}
cor_var <- sum(empates$Tj * (empates$Tj^2 - 1)) / 2
var.cor <- var - cor_var
...
```{r}
var.cor <- var - sum(Empate$Ti * (Empate$Ti^2 - 1)) / 48
...
R <- reclusos %>%
  dplyr::select(rango) %>%
  dplyr::filter(rango != 0)
T <- (sum(abs(R)) - 1) / sqrt(sum(R^2))
p.valor <- pnorm(q = T, lower.tail = F)
paste(
  "El valor del estadístico T =", round(x = T, digits = 4)
)
paste("El p.valor =", round(x = p.valor, digits = 5))
```
<br/>
